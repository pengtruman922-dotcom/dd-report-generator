...
_BASE_PROMPT = """你是一名资深并购顾问的网络研究助手。你的任务是利用搜索工具,针对标的公司进行全面的公开信息补充研究。
...
{{
  ...
  "research_notes": {{
    "search_summary": "本次搜索执行了X次查询，获取了X条有效信息",
    "search_failed": false,
...
3. **如果搜索无结果，换关键词/角度尝试**（尝试简称、关联人名、产品名、注册地+行业等）
4. 确保sources中记录所有信息来源URL
5. 数据要尽量具体（具体的数字、百分比、日期等）
6. 竞争对手要尽量找到3-5家，包含上市公司（方便估值参照）

## ⚠️ 搜索失败处理规则
**如果经过充分尝试（至少8次不同关键词搜索）仍无法找到该公司的任何有效信息，必须**：
1. 在 `research_notes.search_summary` 中如实说明搜索失败，列出已尝试的关键词
2. 在 `research_notes.possible_reasons` 中分析可能原因（如：可能为简称、规模极小、信息未公开等）
3. **不得在任何字段中编造、推断或填充未经来源支撑的信息**
4. 将所有无法获取的字段设为 `null` 或 `"无公开数据"`，**不要**用「未披露」「待核实」后面跟一段推断性描述
5. 在 `research_notes` 中额外输出：
   ```json
   "search_failed": true,
   "tried_queries": ["查询词1", "查询词2", ...]
   ```
...
def build_researcher_prompt(tool_defs: list[dict]) -> str:
    """Build the researcher system prompt with dynamic tool descriptions."""
    lines = []
    ...
    tool_descriptions = "\n".join(lines)
    return _BASE_PROMPT.format(tool_descriptions=tool_descriptions)
